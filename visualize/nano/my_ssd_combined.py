...
def dt_bias_activate(vars):
    if vars["dt_bias"] is not None:
        dt = vars["dt"] + vars["dt_bias"]
    else:
        dt = vars["dt"]
    if vars["dt_softplus"]:
        dt = torch.where(dt < 20.0, torch.nn.functional.softplus(dt), dt)
    if vars["dt_limit"]:
        dt = dt.clamp(min=vars["dt_limit"][0], max=vars["dt_limit"][1])

    return dt


def reconstruct_global_cumsum_from_chunk(dA_cumsum, seqlen):
    """
    Args:
        dA_cumsum: (batch, nheads, nchunks, chunk_size)  -- chunk-wise cumsum returned by Triton kernel
        seqlen: int, true sequence length (no padding)
    Returns:
        global_flat: (batch, nheads, seqlen)  -- the full-sequence cumulative sums
    """
    batch, nheads, nchunks, chunk_size = dA_cumsum.shape
    device = dA_cumsum.device

    # chunk totals: last entry in each chunk (works because padded dt==0 => padded dA==0)
    chunk_totals = dA_cumsum[..., chunk_size - 1]  # (batch, nheads, nchunks)

    # exclusive prefix sum across chunks: sum of all previous chunk totals
    chunk_offsets = (
        torch.cumsum(chunk_totals, dim=2) - chunk_totals
    )  # (batch, nheads, nchunks)

    # add the offset to each time position inside each chunk
    chunk_offsets = chunk_offsets.unsqueeze(-1)  # (batch, nheads, nchunks, 1)
    global_chunk_cumsum = (
        dA_cumsum + chunk_offsets
    )  # (batch, nheads, nchunks, chunk_size)

    # flatten time dimension and trim padding
    global_flat = global_chunk_cumsum.reshape(batch, nheads, nchunks * chunk_size)[
        ..., :seqlen
    ].contiguous()
    return global_flat


def compute_full_diff_from_chunk_cumsum(dA_cumsum, seqlen):
    """
    Returns:
        diff: (batch, seqlen, seqlen, nheads)
        where diff[b,i,j,h] = global_cumsum[b,h,i] - global_cumsum[b,h,j]
    Warning: this materializes the full (batch * seqlen * seqlen * nheads) tensor -> can be huge.
    """
    global_flat = reconstruct_global_cumsum_from_chunk(
        dA_cumsum, seqlen
    )  # (batch, nheads, seqlen)
    # pairwise difference (batch, nheads, seqlen, seqlen)
    diff = global_flat.unsqueeze(-1) - global_flat.unsqueeze(-2)
    # reorder to (batch, seqlen, seqlen, nheads)
    # diff = tmp.permute(0, 2, 3, 1).contiguous()
    return diff


def obtain_vis_attn_and_save(
    dA_cumsum,
    dt,
    B,
    C,
    ...
    """
    Compute full i->j attention per head (and optionally save per-head heatmaps).

    Returns:
      vis_attn_tensor: torch.Tensor (batch, nheads, seqlen, seqlen)
      invalid_mask: np.bool_ (seqlen, seqlen) — True where invalid (upper-triangle, j>=i)

    Notes:
      - dt: (batch, seqlen, nheads)
      - A:  (nheads,)
      - B:  (batch, seqlen, ngroups, dstate)
      - C:  (batch, nchunks_or_seqlen, ngroups, dstate)
      - Produces full (batch, nheads, seqlen, seqlen) attention.
      - Use max_tokens or vis_window to limit seqlen for visualization.
    """
    b2, s2, ngroups, dstate = B.shape
    dt = dt.reshape(b2, -1, dt.shape[-1])[:, :s2, :]
    batch, seqlen, nheads = dt.shape
    assert batch == b2 and s2 == seqlen, f"B shape mismatch {B.shape} vs dt {dt.shape}"
    assert C.shape[0] == batch
    # --- handle C shape ---
    if C.shape[1] == seqlen:
        C_frames = C
    else:
        raise ValueError(f"C shape {C.shape} unsupported; expected seqlen={seqlen}")

    Q = K = seqlen
    B_sel = B
    C_sel = C_frames

    # --- compute group-level dot product ---
    dp = torch.einsum(
        "b k g d, b i g d -> b g i k", B_sel, C_sel
    ).cpu()  # (batch, ngroups, Q, K)

    # --- map heads to groups ---
    if nheads % ngroups != 0:
        raise RuntimeError("nheads must be divisible by ngroups")
    heads_per_group = nheads // ngroups
    group_idx = torch.arange(nheads) // heads_per_group
    dp_heads = (
        dp.unsqueeze(2)
        .expand(-1, ngroups, heads_per_group, -1, -1)
        .reshape(1, nheads, dp.shape[-2], dp.shape[-1])
    )
    # dp_heads = dp[:, group_idx, :, :]  # (batch, nheads, Q, K)
    # --- compute exponential decay weights ---
    # diff = dA_cumsum.unsqueeze(2) - dA_cumsum.unsqueeze(1)  # (batch, seqlen, seqlen, nheads)
    # weight = torch.exp(diff).permute(0, 3, 1, 2).contiguous()  # (batch, nheads, Q, K)
    diff = compute_full_diff_from_chunk_cumsum(
        dA_cumsum.cpu(), seqlen
    )  # (batch, seqlen, seqlen, nheads)
    weight = torch.exp(diff)
    # --- compute attention (before masking) ---
    attn = dp_heads * weight  # (batch, nheads, Q, K)

    # --- causal masking without allocating large mask tensor ---
    # instead of creating a huge mask_keep, use torch.tril_ in-place (saves tons of memory)
    # attn = torch.tril_(attn)  # zero out upper triangle (k >= i)
    attn.tril_()  # in-place, 不会分配新的大块内存

    # --- build invalid mask (for visualization only, small and on CPU) ---
    # invalid_mask = np.triu(np.ones((Q, K), dtype=bool))
    invalid_mask = None
    # --- squeeze batch if needed ---
    vis_attn = attn[0] if batch == 1 else attn

    return vis_attn, invalid_mask
...
def _mamba_chunk_scan_combined_fwd(
    ...
    """x.shape: (batch, seqlen, nheads, headdim)"""
    batch, seqlen, nheads, headdim = x.shape
    _, _, ngroups, dstate = B.shape
    assert nheads % ngroups == 0
    assert B.shape == (batch, seqlen, ngroups, dstate)
    ...
    if return_attention:
        # dt = dt_bias_activate({"dt": dt, "dt_bias": dt_bias, "dt_softplus": dt_softplus, "dt_limit": dt_limit})
        vis_attn, invalid_mask = obtain_vis_attn_and_save(dA_cumsum, dt, B, C)
    ...
    if cu_seqlens is None:
        if return_attention:
            return out, out_x, dt, dA_cumsum, states, final_states, vis_attn
        ...
        if not return_varlen_states:
            if return_attention:
                ...
                return (
                    ...
                    else (out, final_states, vis_attn)
                )
