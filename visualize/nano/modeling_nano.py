...
Modify for supporting data packing
"""
...
from .configuration_nano import NemotronHConfig
...
class NemotronHMamba2Mixer(nn.Module):
    ...
    #     seq_idx: Optional[torch.LongTensor] = None,
    ...
    def cuda_kernels_forward(
        ...
        # Fused calculations or step by step if no initialized cache is found
            ...
            if self.training and cache_params is None:
                ...
                # 3. SSM transformation
                scan_output, ssm_state, *rest = mamba_chunk_scan_combined(
                    hidden_states.view(batch_size, seq_len, -1, self.head_dim),
                    dt,
                    A,
                    B.view(batch_size, seq_len, self.n_groups, -1),
    ...
    def torch_forward(
        ...
        if use_precomputed_states:
            ...
            # B: [bsz, num_chunks, chunk_size, num_heads, state_size]
            # C: [bsz, num_chunks, chunk_size, num_heads, state_size]
            # hidden_states: [bsz, num_chunks, chunk_size, num_heads, head_dim]
            ...
            L = torch.exp(segment_sum(A)) # [bsz, num_heads, -1, chunk_size, chunk_size]

            # Contraction of C and B to get G (attention-weights like)
            G_intermediate = C[:, :, :, None, :, :] * B[:, :, None, :, :, :]  # shape: (b, c, l, s, h, n)
            G = G_intermediate.sum(dim=-1)  # shape: (b, c, l, s, h) == 1, 17, 128, 128, 128 # [bsz, num_chunks, chunk_size, chunk_size, num_heads]

            # Compute M, equivalent to applying attention mask to weights
            M_intermediate = G[..., None] * L.permute(0, 2, 3, 4, 1)[..., None]
            M = M_intermediate.sum(dim=-1) # [bsz, num_chunks, chunk_size, chunk_size, num_heads]

            # Compute Y_diag (apply to values), contracting the ssm_state_size dimension
            Y_diag = (M[..., None] * hidden_states[:, :, None]).sum(dim=3) # hidden state: 17, 128, 128, 80， Y_diag: [bsz, num_chunks, chunk_size, num_heads, head_dim]
        ...
        with torch.cuda.stream(torch.cuda.default_stream(hidden_states.device)):
            ...
            if self.block_type == "mamba":
                hidden_states = self.mixer(
                    ...
                    seq_idx=seq_idx,  # supporting data packing
                ...
                hidden_states, attention_weights = hidden_states[0], hidden_states[1]
            elif self.block_type == "mlp":
            ...
            return hidden_states, (
                attention_weights if self.block_type in ["attention", "mamba"] else None
            )
    ...
    def forward(
        ...
        **kwargs,
        ...
        attn_output = _flash_attention_forward(
            ...
            key_states,
            value_states,
            attention_mask=None,  # here attention_mask is used for padding tokens, but our attention mask is used for causal masking
            query_length=q_len,
            position_ids=position_ids,
...
print("Successfully registered 'NemotronHEagerAttention' with key 'eager_manual'.")


class NemotronHEagerAttention(NemotronHAttention):
    """
    An Eager implementation of NemotronHAttention that manually calculates
    attention scores to expose the attention weights for visualization.
    It inherits from NemotronHAttention to reuse the __init__ method.
    """
    ...
    def forward(
        ...
        position_ids: Optional[torch.LongTensor] = None,
        past_key_value: Optional[HybridMambaAttentionDynamicCache] = None,
        output_attentions: bool = False,  # This flag is crucial
        use_cache: bool = False,
        cache_position: Optional[torch.LongTensor] = None,
    ) -> Tuple[torch.Tensor, Optional[torch.Tensor], Optional[Tuple[torch.Tensor]]]:

        # If attentions are not requested, we can optionally fall back to the superclass's
        # optimized version for consistency, but for this specific purpose, we will
        # always compute manually to demonstrate the process. The key is that this
        # implementation CAN produce the weights.
        ...
        # === MANUAL ATTENTION CALCULATION START ===
        ...
        # 3. Apply the attention mask (additive mask)
        if attention_mask is not None:
            # Ensure the mask covers the correct sequence length
            causal_mask = attention_mask[:, :, :, : key_states.shape[-2]]
            attn_weights = attn_weights + causal_mask

        # 4. Softmax to get probabilities (this is our attention matrix!)
        # Using float32 for softmax is a good practice for numerical stability
        attn_weights = torch.nn.functional.softmax(
            attn_weights, dim=-1, dtype=torch.float32
        ).to(query_states.dtype)

        # 5. Optional: Apply dropout (usually skipped during inference/visualization)
        # attn_weights = torch.nn.functional.dropout(attn_weights, p=self.attention_dropout, training=self.training)

        # 6. Calculate final output: Attention_Weights @ V
        attn_output = torch.matmul(attn_weights, value_states)

        # === MANUAL ATTENTION CALCULATION END ===
        ...
        # Return attn_weights along with the output
        # If output_attentions is False, we should return None for the weights
        if not output_attentions:
            attn_weights = None
...
class NemotronHModel(NemotronHPreTrainedModel):
        ...
        **kwargs,
        ...
        for layer_idx, mixer_block in enumerate(self.layers):
            ...
            if mixer_block.block_type == "attention":
                if output_attentions and hidden_states[1] is not None:
                    # append attentions only of attention layers. Mamba layers return `None` as the attention weights
                    all_self_attns += ((hidden_states[1], "attention"),)
            elif mixer_block.block_type in ["mamba"]:
                if output_attentions and hidden_states[1] is not None:
                    # append attentions only of attention layers. Mamba layers return `None` as the attention weights
                    all_self_attns += ((hidden_states[1], "mamba"),)
            else:
                if output_attentions:
                    all_self_attns += ((None, None),)
            hidden_states = (
    ...
    def _update_causal_mask(self, attention_mask, input_tensor, cache_position):
        """
        (Pdb) (attention_mask == 1).sum()
        tensor(3023453, device='cuda:0')
        (Pdb) (attention_mask==0).sum()
        tensor(45181796, device='cuda:0')

        (Pdb) (causal_mask==0).sum()
        tensor(24106096, device='cuda:0')
        """
        if attention_mask is None:
            ...
            if self.config._attn_implementation in ["flash_attention_2"]:
                raise ValueError(
                    "attention_mask must be None if using flash_attention_2"
                )
            else:
                return attention_mask
...
AutoConfig.register("nano", NemotronHConfig, exist_ok=True)
AutoModelForCausalLM.register(NemotronHConfig, NemotronHForCausalLM, exist_ok=True)
