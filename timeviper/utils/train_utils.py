...
def load_image(image_file):
    ...
    if not image_file.exists() and not image_file.is_file():
        if all(
            [not image_file.with_suffix(post_fix).exists() for post_fix in post_fixs]
        ):
...
def load_images(image_files):
    ...
    for image_file in tqdm(
        image_files, desc="Loading images", disable=len(image_files) < 1000
    ):
        if isinstance(image_file, Image.Image):
            image = image_file
        else:
...
def load_json_data(data_file):
    ...
    if data_file.endswith(".json"):
        ...
        raise ValueError(
            f"Unsupported file format: {data_file}. Only .json and .jsonl are supported."
        )
    return data
...
def get_image_attention_mask(
    output_input_ids, max_num_images, tokenizer, include_image=True
):
...
def verify_yaml(args):
    ...
    result = subprocess.run(
        ["pytest", "-m", "prerun", f"--yaml-path={args.training_data_yaml}"]
    )
...
def save_checkpoint(
    epoch, model, args, accelerator, unwrapped_model=None, global_step=None
):
...
def save_final_weights(model, args, accelerator, processor=None, tokenizer=None):
    ...
    if args.save_hf_model:
        ...
        if (
            ...
            checkpoint_dict = {
                k: v for k, v in checkpoint_dict.items() if k in trainable_params_name
            }
...
def find_and_remove_tokens(
    input_tensor, labels_tensor, attention_mask_tensor, token_id, tokenizer
):
    ...
    for i in range(batch_size):
        ...
        new_single_attention_mask = torch.masked_select(
            single_attention_mask, single_input != token_id
        )
    ...
    new_input = torch.nn.utils.rnn.pad_sequence(
        new_input_list, batch_first=True, padding_value=tokenizer.pad_token_id
    )
    new_labels = torch.nn.utils.rnn.pad_sequence(
        new_labels_list, batch_first=True, padding_value=-100
    )
    new_attention_mask = torch.nn.utils.rnn.pad_sequence(
        new_attention_mask_list, batch_first=True, padding_value=0
    )
...
def _right_pad_inputs_with_attention_mask(model_inputs: List[Dict]):
    ...
    assert (
        len(model_inputs) == 1
    ), "This method only supports a single input, but get {} inputs".format(
        len(model_inputs)
    )
    for k in model_inputs[0].keys():
        if k == "pixel_values" and isinstance(model_inputs[0][k], list):
            results[k] = [
                inputs[k] if inputs[k] is not None else None for inputs in model_inputs
            ]
