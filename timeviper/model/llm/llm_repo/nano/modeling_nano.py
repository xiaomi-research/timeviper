...
class NemotronHModel(NemotronHPreTrainedModel):
    def __init__(self, config):
        ...
        # for pdrop
        self.use_pdrop = config.use_pdrop

        self.pdrop_args = {"use_pdrop": config.use_pdrop}
        if self.use_pdrop:
            assert (
                config.pdrop_type is not None
        ...
        merge_module_name = config.merge_module
        if merge_module_name == "CrossAttention":
            ...
            for pdrop_layer_idx, _ in enumerate(
                ...
                if "drop" in self.pdrop_args["pdrop_compress_types"][pdrop_layer_idx]:
            ...
            raise ValueError(f"Invalid merge module name: {merge_module_name}")
    ...
    def forward(
        ...
        attention_mask: Optional[torch.Tensor] = None,
        **kwargs,
    ) -> Union[Tuple, BaseModelOutputWithPast, BaseModelOutputWithPastAndLabels]:
        output_attentions = (
            output_attentions
        ...
        for layer_idx, mixer_block in enumerate(self.layers):
            ...
            if self.use_pdrop:
                ...
                if (
                    rank_layer in self.pdrop_layers
                    and train_pdrop_args["is_interleaved"] is False
                ):
                    stage = self.pdrop_layers.index(rank_layer)
                    if hidden_states.shape[1] != 1:  # prefill stage or training
                        ...
                        cur_visual_length = [
                            int(cur_vision_token * self.pdrop_ratios[stage])
                            for cur_vision_token in kwargs["train_pdrop_args"][
                                "num_vision_tokens"
                            ]
                        ]
                        next_visual_length = [
                            int(cur_vision_token * self.pdrop_ratios[stage + 1])
                            for cur_vision_token in kwargs["train_pdrop_args"][
                                "num_vision_tokens"
                            ]
                        ]
                        ...
                        for idx, cur_position_ids in enumerate(position_ids):
                            cur_position_ids = cur_position_ids - (
                                cur_visual_length[idx] - next_visual_length[idx]
                            )
                            new_position_ids.append(cur_position_ids)
                        assert idx == 0, idx
                        position_ids = torch.tensor(
                            new_position_ids, dtype=torch.long
                        ).unsqueeze(0)
            if torch.isnan(hidden_states).any():
                raise ValueError("NaN detected in hidden_states before mixer block")
        ...
        if self.use_pdrop:
            if not return_dict:
                return tuple(
                    v
                    for v in [hidden_states, past_key_values, all_hidden_states, labels]
                    if v is not None
                )
    ...
    def pdrop_no_pack(
        ...
        if "attn" in pdrop_compress_type:
            ...
            assert self.layers[rank_layer].block_type == "attention"
            self_attn = self.layers[rank_layer].mixer
        ...
        for i in range(batch_size):
            ...
            if "attn" in pdrop_compress_type:
                ...
                if self.training:
                    ...
                    index_before_answer = index_before_answer[
                        -1
                    ]  # take the last one for multi-turn case
                    index_before_answer = torch.tensor(
                        index_before_answer, device=labels[0].device
                    )
            ...
            # TODO: do not support multi-image or interleaved vision tokens yet
            start_index = vision_index + image_tokens[i]
            if (
                self.merge_modules is not None
                and self.merge_module_names[cur_num] != "none"
            ):
    ...
    def pdrop_pack(
        ...
        rank_layer: int,
        pdrop_compress_type,
        seq_idx,  # kept for API-compatibility; not used inside
        train_pdrop_args,
        ...
        # update sample len for next time dropping
    ...
    def flash_rank_drop(
        ...
        pdrop_compress_type = self.pdrop_compress_types[cur_num]
        if position_ids is None:
        ...
        if is_packed:
            ...
            return self.pdrop_no_pack(
                ...
                train_pdrop_args["first_vision_token_positions"],
                train_pdrop_args["num_vision_tokens"],
                train_pdrop_args["text_prompt_lens"],
            )
...
class NemotronHForCausalLM(NemotronHPreTrainedModel, GenerationMixin):
    ...
    def forward(
        ...
        **kwargs,  # for now we need this for generation
        ...
        if self.backbone.use_pdrop and labels is not None:
            labels = nemotron_h_outputs[-1]  # may be updated by pdrop
    ...
    def init_cross_attn_from_self_attn(self):
        if self.backbone.merge_modules is not None:
            for idx, module in enumerate(self.backbone.merge_modules):
                pdrop_layers = self.backbone.pdrop_args.get("pdrop_layers", [])
                try:
                    module.load_state_dict(
                        self.backbone.layers[pdrop_layers[idx]].mixer.state_dict()
                    )
        ...
        if self.backbone.merge_modules is not None:
            for idx, module in enumerate(self.backbone.merge_modules):
                ...
                for layer_idx in range(pdrop_layers[idx], pdrop_layers[idx] + 20):
                    try:
                        module.load_state_dict(
                            self.backbone.layers[layer_idx].mixer.state_dict()
                        )
                        break
                    except Exception as e:
    ...
    def init_mlp_from_llm(self):
        if self.backbone.merge_ffn_modules is not None:
            for idx, module in enumerate(self.backbone.merge_ffn_modules):
                pdrop_layers = self.backbone.pdrop_args.get("pdrop_layers", [])
                for layer_idx in range(pdrop_layers[idx], pdrop_layers[idx] + 4):
                    try:
                        print("loading MLP weights from layer:", layer_idx)
                        module.load_state_dict(
                            self.backbone.layers[layer_idx].mixer.state_dict()
                        )
                        break
