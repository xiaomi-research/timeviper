...
class LetterboxPad:
    ...
    def __call__(self, image: Image) -> Image:
        ...
        return TVF.pad(
            image, padding, fill=self.padding_fill_value, padding_mode="constant"
        )
...
class VisionBackbone(nn.Module, ABC):
    ...
    def get_identifier(self) -> str:
        return "vanilla-vision-backbone"
...
class TimmViTBackbone(VisionBackbone, ABC):
    def __init__(
        ...
        if use_zero3:
            transforms_list = list(default_image_transform.transforms)
            for i, t in enumerate(transforms_list):
                if isinstance(t, torchvision.transforms.Normalize):
                    transforms_list[i] = Normalize(mean=t.mean.cpu(), std=t.std.cpu())
                    print(
                        f"Replaced torchvision.transforms.Normalize with LazyNormalize."
                    )
            else:
                print(
                    "Warning: Could not find torchvision.transforms.Normalize to replace."
                )

            default_image_transform = Compose(transforms_list)
        ...
        if "siglip" in self.timm_path_or_url or "in1k" in self.timm_path_or_url:
            ...
            assert isinstance(
                resize_transform := default_image_transform.transforms[0], Resize
            )
            default_image_transform = Compose(
                [
                    Resize(
