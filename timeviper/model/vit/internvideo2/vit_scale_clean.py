...
class PretrainVisionTransformer_clean(nn.Module):
    def __init__(
        ...
        if sep_pos_embed:
            ...
            if sep_image_video_pos_embed:
                logger.info(
                    "Use separate position embedding, for image and video we use different pos_embed."
                )
        ...
        self.init_pos_embed()
        trunc_normal_(self.cls_token, std=0.02)  # NOTE 对chat没用，都要加载预训练的
    ...
    def forward(self, x, mask=None, use_image=False):
        ...
        B, T, L, C = x.shape  # T: temporal; L: spatial, torch.Size([24, 1, 256, 1408])
        x = x.view([B, T * L, C])
...
def pretrain_internvideo2_giant_patch14_224_clean(config):
    ...
    if config.vision_encoder.pretrained is not None:
        logger.info(
            f"Loading pretrained weights from {config.vision_encoder.pretrained}"
        )
        state_dict = torch.load(config.vision_encoder.pretrained, map_location="cpu")
        interpolate_pos_embed_internvideo2(
            state_dict, model, orig_t_size=8
        )  # NOTE 8f for stage1
